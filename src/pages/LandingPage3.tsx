    ...
    try {
      ...
      if (response.ok) {
        ...
        const justdialJobs = data.filter((job: Job) => {
          ...
          if (isJustdial) {
            console.log(`✓ Found Justdial job: "${job.job_title}" - "${job.company_name || job.company}"`);
          }

          return isJustdial;
        });

        console.log("Justdial jobs:", justdialJobs);
        console.log("Justdial jobs count:", justdialJobs.length);

        setJobs(justdialJobs);
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform hover:scale-[1.01] transition-transform duration-300">
          {/* Header Section */}
          <div className="bg-gradient-to-r from-purple-600 via-pink-600 to-rose-600 p-8 text-center relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-12 text-base border-2 border-gray-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition-all duration-200 rounded-xl"
                required
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className="h-12 text-base border-2 border-gray-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 rounded-xl">
                ...
                <SelectContent>
                  {jobs.length === 0 && !loadingJobs ? (
                    ...
                    jobs.map((job) => (
                      <SelectItem key={job.id} value={job.id.toString()} className="text-base py-3 cursor-pointer hover:bg-purple-50">
                        {job.job_title} - {job.company_name} ({job.type || 'Full Time'})
                      </SelectItem>
                    ))
                  )}
                </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <label
                  htmlFor="resume"
                  className="flex items-center justify-center h-12 px-4 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-purple-500 hover:bg-purple-50 transition-all duration-200 group"
                >
                  <Upload className="h-5 w-5 text-gray-400 mr-2 group-hover:text-purple-500 transition-colors" />
                  <span className="text-base text-gray-600 group-hover:text-purple-600 transition-colors">
                    {formData.resume ? formData.resume.name : "Choose file"}
                  </span>
                </label>
                {formData.resume && (
                  <div className="mt-2 text-sm text-green-600 flex items-center animate-in slide-in-from-top-2 duration-300">
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-14 text-lg font-semibold bg-gradient-to-r from-purple-600 via-pink-600 to-rose-600 hover:from-purple-700 hover:via-pink-700 hover:to-rose-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 rounded-xl transform hover:scale-[1.02] active:scale-[0.98]"
            >
              {loading ? (
        ...
        <div className="absolute top-10 left-10 w-20 h-20 bg-purple-200 rounded-full opacity-20 blur-xl animate-pulse"></div>
        <div className="absolute bottom-10 right-10 w-32 h-32 bg-pink-200 rounded-full opacity-20 blur-xl animate-pulse delay-75"></div>
      </div>
    </div>
  );
};
